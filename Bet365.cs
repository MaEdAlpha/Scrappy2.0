    ...
    
{
    ...
    
    {
        ...
        private static string strCurrentLeagueSource = "";
        private static bool UseInitialTab = true;
        ...
        public static void ScrapeHA()
        {
            //Reset the timer so we get the time per full cycle
            ...
            {
                //Get the league item from the list
                var item = cycleList.ElementAt(i);

                string divisionTitle = item.Value;
                string leagueTitle = item.Key;

                GetLeagueTab(item.Key, item.Value);

                GetLeagueDataGecko(DATERANGE, leagueTitle, divisionTitle);

            }
        }

        private static void GetLeagueTab(string League, string LeagueURL)
        {
            //Check to see if the current league already has a tab open 
            string strTempValue;
            

            if (TabRefList.TryGetValue(League, out strTempValue))
            {
                //If it does set the focus to that tab
                driver.SwitchTo().Window(strTempValue);
                strCurrentLeagueSource = driver.PageSource;
            }
            else
            {
                ...
                //If first league then use exisiting tab else open a new one
                if (UseInitialTab == true)
                {
                ...
                { 
                    ((IJavaScriptExecutor)driver).ExecuteScript("window.open(\'" + LeagueURL + "\')"); 
                }
      
                //Ensure that we are looking at the new tab
                driver.SwitchTo().Window(driver.WindowHandles.Last());

                strCurrentLeagueSource = driver.PageSource;

                //Add the handler and league to the dictionary. 
                TabRefList.Add(League, driver.CurrentWindowHandle);
        ...
        public static void GetLeagueDataGecko(int DATERANGE, string leagueTitle, string divisionTitle)
        {
            {
                string tempDate = ""; //store date here to write to list later
                string finalDate = "";
                ...
                string forAllMatches = "//div[contains(@class, 'sgl-MarketFixtureDetailsLabel gl-Market_General gl-Market_General-columnheader gl-Market_General-haslabels ')]/child::div";
                var MatchesNodes = GetElements(strCurrentLeagueSource, forAllMatches);

                try
                {
                    foreach (var matchWebElement in MatchesNodes)
                    {

                        //Gets IWebElement within specified day range.
                        if (MatchDateRangeGecko(matchWebElement.InnerText) <= dayRange && (matchWebElement.InnerText.StartsWith("Mon") || matchWebElement.InnerText.StartsWith("Tue") || matchWebElement.InnerText.StartsWith("Wed") || matchWebElement.InnerText.StartsWith("Thu") || matchWebElement.InnerText.StartsWith("Fri") || matchWebElement.InnerText.StartsWith("Sat") || matchWebElement.InnerText.StartsWith("Sun")))
                        {
                            tempDate = matchWebElement.InnerText;
                        }
                        //Breaks from looop when it finds date range out of spec
                        else if (MatchDateRangeGecko(matchWebElement.InnerText) > dayRange && (matchWebElement.InnerText.StartsWith("Mon") || matchWebElement.InnerText.StartsWith("Tue") || matchWebElement.InnerText.StartsWith("Wed") || matchWebElement.InnerText.StartsWith("Thu") || matchWebElement.InnerText.StartsWith("Fri") || matchWebElement.InnerText.StartsWith("Sat") || matchWebElement.InnerText.StartsWith("Sun")))
                        {
                            break;
                        }
                        //Match Time
                        else if (matchWebElement.InnerText.Contains(":"))
                        {
                            var TestForInplay = matchWebElement.SelectNodes(".//div[@class='pi-ScoreVariantSingleParticipant-team1 ']");

                            //If the game is not inplay 
                            if (TestForInplay == null)
                                {

                                string Time = matchWebElement.SelectSingleNode(".//div[@class='rcl-ParticipantFixtureDetails_BookCloses ']").InnerHtml;
                                string ScrapedHomeTeam = matchWebElement.SelectSingleNode("(.//div[@class='rcl-ParticipantFixtureDetails_Team '])[1]").InnerHtml;
                                string ScrapedAwayTeam = matchWebElement.SelectSingleNode("(.//div[@class='rcl-ParticipantFixtureDetails_Team '])[2]").InnerHtml;

                                string[] matchDetails = { Time, ScrapedHomeTeam, ScrapedAwayTeam };
                                ...
                                List<string> filteredOddsResults = GetStuffedB365Gecko(); //Checks for postponed matches

                                string index = (pathCount + 1).ToString(); //pathCount is a tracker for what match we are on. Postponed matches are still part of this list. We need to determine a way to detect them.
                                                                     //Compare this index string to the filtered ShoveItB365Odds() array. If index == elm.index && elm[index] == null (postponed match) do nothing. 


                                string xPathForOdds = GetOddsXpath(index);
                                var OddsNodes = GetElements(strCurrentLeagueSource, xPathForOdds);
                                ...
                                if (filteredOddsResults == null)
                                {
                                    tempHomeOdds = OddsNodes[pathCount].InnerText;
                                    tempDrawOdds = OddsNodes[((OddsNodes.Count / 3)) + pathCount].InnerText;
                                    tempAwayOdds = OddsNodes[((OddsNodes.Count / 3 * 2)) + pathCount].InnerText;
                                    //Console.WriteLine("In non Postponed odds write");
                                }
                                else
                                {
                ...
                {
                    Console.WriteLine("\n Total Matches found for " + leagueTitle + " : {0} \n", leagueMatchCount);
        ...
        private static List<string> GetStuffedB365Gecko()
        {
            if (GetElements(strCurrentLeagueSource, "//div[contains(@class, 'postponed')]") != null)
            {
                var allOddsFixtures = GetElements(strCurrentLeagueSource, "//div[contains(@class, 'sgl-MarketOddsExpand gl-Market_General gl-Market_General-columnheader ')]/div");
                int totalElm = allOddsFixtures.Count(); // gives you total count. We will later divide by 3, to get the total rows in each column of '1' 'X' '2'.
                List<string> indexFilterList = new List<string>();

                for (int i = 0; i < totalElm; i++)
                {
                    //Xpath for condition where '1' 'X' '2' exist.
                    if (allOddsFixtures[i].InnerText.Length == 1)
                    {
                    ...
                    else if (allOddsFixtures[i].InnerText.Length > 4 || allOddsFixtures[i].InnerText.Length == 0)
                    {
                        indexFilterList.Add("999");
                        //Console.Write(i + " Postponed\n");

                    }
                    else if (allOddsFixtures[i].InnerText.Length == 4)
                    {
                        //Xpath;
                        ...
                        indexFilterList.Add(allOddsFixtures[i].InnerText);
        ...
        private static double MatchDateRangeGecko(string matchWebElement) //Compare match dates with current date UTC.
        {
            ...
            string dayInt = matchWebElement.Substring(4, 2);
            string month = matchWebElement.Substring(7, 3);
            string date;

            //Check for new Year
            //We'll account for a 5 day window. So if year today is different to the year in 5 days time then check if the game is in Jan. If it is add 1 year
            if (DateTime.Now.AddDays(5).Year != DateTime.Now.Year)
            {
                if (month == "Jan")
                {
                    date = month + "" + dayInt + ", " + (DateTime.Now.Year + 1);
                }
                else
                {
                    date = month + "" + dayInt + ", " + DateTime.Now.Year;
                }
        ...
        public static void InitiateListB365HA()
        {
            //Hard code the URl in here for now
            ...
            leagueDivisionDict.Add("England Premier League", "https://www.bet365.com/#/AC/B1/C1/D13/E51761579/F2/");
            ...
            leagueDivisionDict.Add("Spain Primera Liga", "https://www.bet365.com/#/AC/B1/C1/D13/E52115687/F2/");
            leagueDivisionDict.Add("Germany Bundesliga I", "https://www.bet365.com/#/AC/B1/C1/D13/E51669667/F2/");
            leagueDivisionDict.Add("Italy Serie A", "https://www.bet365.com/#/AC/B1/C1/D13/E52224631/F2/");
        ...
        {
            Console.WriteLine("////////////////////////////////////////////////////////////////////////////////////////////////////\n\nLeague List Cycle: {0} \n Total Matches Scraped = {1}\nTotal Time Duration: {2} seconds \n\n////////////////////////////////////////////////////////////////////////////////////////////////////", masterCount, tmsCount, Math.Round((DateTime.UtcNow - timeZero).TotalSeconds));
